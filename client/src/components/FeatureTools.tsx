...
export function FeatureTools({ onAssistantMessage }: Props) {
  ...
  return <div className="mb-4 grid gap-3 md:grid-cols-3">
    <div className="rounded-2xl border border-[#e4ddd3] bg-white/70 p-3 dark:border-white/15 dark:bg-white/[0.05]">
      <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-[#586373] dark:text-[#c8d1df]"><Newspaper className="size-4 text-[#ef795f]" /> Briefing berita</div>
      <div className="flex gap-2"><input value={newsTopic} onChange={(event) => setNewsTopic(event.target.value)} placeholder="Topik: teknologi, bisnis..." className="min-w-0 flex-1 rounded-lg border border-[#e4ddd3] bg-white px-2 py-1.5 text-xs outline-none dark:border-white/15 dark:bg-white/10" /><button onClick={() => news.mutate({ topic: newsTopic || undefined })} disabled={news.isPending} className="rounded-lg bg-[#182536] px-2 text-xs text-white disabled:opacity-50">{news.isPending ? "..." : "Ringkas"}</button></div>
    ...
    <div className="rounded-2xl border border-[#e4ddd3] bg-white/70 p-3 dark:border-white/15 dark:bg-white/[0.05]">
      ...
      {imageUrl && <div className="mt-2"><img src={imageUrl} alt="Hasil generate foto" className="max-h-32 w-full rounded-lg object-cover" /><a href={imageUrl} download target="_blank" rel="noreferrer" className="mt-1 block text-xs text-[#c95d47] underline">Buka atau download hasil</a></div>}
    </div>
    <div className="rounded-2xl border border-[#e4ddd3] bg-white/70 p-3 dark:border-white/15 dark:bg-white/[0.05]">
      ...
      <div className="mb-2 flex gap-2"><select value={language} onChange={(event) => setLanguage(event.target.value)} className="rounded-lg border border-[#e4ddd3] bg-white px-2 py-1.5 text-xs dark:border-white/15 dark:bg-white/10"><option value="auto">Auto</option><option value="typescript">TypeScript</option><option value="python">Python</option><option value="javascript">JavaScript</option><option value="sql">SQL</option></select><span className="self-center text-[10px] text-[#9a938b]">Tidak menjalankan kode</span></div>
